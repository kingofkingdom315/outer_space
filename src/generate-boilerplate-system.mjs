...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'outer_space', 'outer_space', and 'outer_space'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.outer_space/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.outer_space/g),
   ...
   * Rename files.
   *
   * Rename files that had outer_space in their name, such as
   * css/outer_space.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*outer_space*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('outer_space', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-outer_space-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob outer_space's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace outer_space name mentions in files.
      generator.replaceFileContents();
      // Rename files that had outer_space in their name.
